...
void visualization(PointCloudPtr cloud_src, PointCloudPtr cloud_tar, PointCloudPtr keyPoint_src, PointCloudPtr keyPoint_tar, Eigen::Matrix4f Mat, float resolution)
{
	...
	pcl::visualization::PointCloudColorHandlerCustom< pcl::PointXYZ> cloud_color_handler_src(cloud_src, 255, 0, 0);
	pcl::visualization::PointCloudColorHandlerCustom< pcl::PointXYZ> cloud_color_handler_tar(cloud_tar, 0, 0, 255);
    ...
    cout << "NOTICE: Press space to register." << endl;
	while (!viewer.wasStopped())
	{
		viewer.spinOnce();
		//transform
